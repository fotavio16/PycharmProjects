...
# Example 1-8: Vector Motion Acceleration
...
class Vector():
    ...
    def limite(self, valor):
        if self.mag() > valor:
            self.vector[0] = self.vector[0] * valor / self.mag()
            self.vector[1] = self.vector[1] * valor / self.mag()
...
class Mover():
    def __init__(self, cor):
        ...
        self.acceleration = Vector(random.uniform(-0.01, 0.01), random.uniform(-0.01, 0.01))
        self.topspeed = 10
        self.cor = cor

    def update(self):
        self.velocity.add(self.acceleration)
        self.velocity.limite(self.topspeed)
